...
$(function(){
	$("#sginAccount").click(function(){
		...
		var Status=$("#sginStatus").css({background:"url('"+BASE_PATH+"/image/loading4.gif') no-repeat 180px 0"}).show(200);
		...
		$.ajax({
			...
			success : function(text) {
				...
				if (text.isEmpty()=="sgin is error") {
					Status.css({background:"url('"+BASE_PATH+"/image/pointError.png') no-repeat 90px 0"});
					span.text("用户名密码错误").css({marginRight:'100px'});
				} else {
					_docForm.password.value=text;
					Status.css({background:"url('"+BASE_PATH+"/image/pointYes.png') no-repeat 110px 0"});
